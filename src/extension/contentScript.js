...
(function() {
  function extractListings() {
    ...
    if (!items.length) {
      // try a fallback selector for listing rows
  ...
  function annotateElement(el, signal, valuation) {
    ...
    // Try to place badge in top-right of element
  ...
  async function processListings() {
    ...
    for (const item of items) {
      try {
        if (!item.id) continue;
        // Send message to background to get valuation
